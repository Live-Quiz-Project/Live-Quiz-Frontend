...
export default function FillBlank({ timeLeft }: Props) {
  const dispatch = useDispatch<StoreDispatch>();
  const auth = useTypedSelector((state) => state.auth);
  const mod = useTypedSelector((state) => state.mod);
  const [answeredOptions, setAnsweredOptions] = useState<TextOption[]>(
    (mod.value.question!.options as TextOption[]).map((option) => ({
      ...option,
      content: "",
    }))
  );
    ...
    dispatch(
      trigger({
        ...
        payload: {
          ...
          time: Math.round((mod.value.question!.timeLimit - timeLeft) * 10),
          options: answeredOptions.map((option) => ({
            id: option.id,
        ...
        >
          {(!mod.value.answers ||
            ...
            <Unanswered
              ...
              setAnsweredOptions={setAnsweredOptions}
            />
          )}
          {mod.value.answers && mod.value.answers.answers.length > 0 && (
            <Answered />
