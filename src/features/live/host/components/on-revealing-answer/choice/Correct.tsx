...
export default function Correct({ setMode }: Props) {
  ...
  return (
    <div className="bg-quartz grid grid-rows-[auto_1fr] gap-[1em] justify-items-center items-center h-full p-4 xs:p-6 md:p-8 lg:p-12 2xl:p-[2.5vw]">
      <div className="grid grid-cols-[1fr_auto_1fr] gap-[1em] items-center justify-items-center w-full">
        <FilledButton
        ...
        <FilledButton
          onClick={() => setMode((prev) => prev + 1)}
          className="font-sans-serif bg-dune text-beige !p-2 xs:!px-5 xs:!py-3 text-[body-1 md:text-header-2] 2xl:text-[1vw] h-fit w-max self-center place-self-end"
        >
          Next
        </FilledButton>
      </div>
      ...
      >
        <BaseAccordion.Head>
          Correct answer{mod.value.answers.answers.length > 0 ? "s" : ""}
        </BaseAccordion.Head>
